...
import com.beicai.common.{GlobalConstants, LogConstants}
...
object AnalysisLogTask {
    def processArgs(sparkConf: SparkConf,args:Array[String])={
      if(args.length >= 1 && Utils.validateDate(args(0))){
        ...
        println(
          """
            |参数有误：
            |com.beicai.music.task.analysislog.AnalysisLogTask需要传入一个 "yyyy-MM-dd" 格式的时间参数
            | 这个参数用于程序解析这个时间的日志.
          """.stripMargin)
        System.exit(0)
      }
  ...
  def processInputPath(sparkConf: SparkConf)={
    ...
    val inputPath = new Path(GlobalConstants.HDFS_LOG_PATH_PREFIX.concat(datePath))
  ...
  def main(args: Array[String]): Unit = {
    ...
    //加载ip规则库
    val ipRules: Array[IPRule] = sparkSession.read.textFile("/mr/input/ip.data").map(line => {
