package com.beicai.task.retention

import com.beicai.common.LogConstants
import com.beicai.util.{JedisUtils, Utils}
...
object RetentionRateTask {
  ...
  def validateArgs(args: Array[String]) = {
    ...
    println(
      ...
      //2018-05-09 00:00:00
      //System.currentTimeMillis()==>2018-05-09 10:49:26
      //当前日期
  ...
  def loadDataFromHbase(startTime: Long, endTime: Long, sparkSession: SparkSession) = {
    println(s"正在加载${Utils.formatDate(startTime,"yyyy-MM-dd HH:mm:ss" )} 到 ${
      Utils.formatDate(endTime,"yyyy-MM-dd HH:mm:ss")
    }的数据")
    //初始化一个scan
    ...
    val userRDD = sparkSession.sparkContext.newAPIHadoopRDD(jobConf,classOf[TableInputFormat],classOf[ImmutableBytesWritable],
      ...
      val result = t2._2
      val deviceId = Bytes.toString(result.getValue(LogConstants.LOG_HBASE_TABLE_FAMILY.getBytes(),LogConstants
        .LOG_COLUMNS_NAME_DEVICE_ID.getBytes()))
      (deviceId,deviceId)
    })
    userRDD
  }
   ...
   //基准日的开始时间和结束时间
    ...
    if(todayUserCount != 0){
     val value = Utils.getScale(nextDayUserCount.toDouble / todayUserCount ,2)
      ...
      val filed = runDate
      //将结果写入redis
      JedisUtils.hset(key,filed,value.toString())
